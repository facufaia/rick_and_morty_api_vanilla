...
function HTMLCard(character,container){
            ...
            zx.forEach(element => {
                ...
                else{element.style.background = "#A6A6A6";}
            });
...
function HTMLTitle(data){
    main__title.innerHTML =    `<h1 class="main__h1">Location: <span style="color: rgb(13,110,253);">${data.name}</span></h1>
...
const apiOnload=async(id)=>{
    let url;
    if(isFinite(id)){url=`https://rickandmortyapi.com/api/location/${id}`;}
    else{url=`${id}`;}
...
let array=[]
for (let i = 1; i < 127; i++) {array.push(i);
};
api(array);
function listener(){
