...
function changeCharacterStatus(characters){
    characters.forEach(character => {
        ...
        else{character.style.background = "#A6A6A6";};
    });
}
function listenClickCard(card,character){
    card.addEventListener("click",()=>{
        ...
        modal.addEventListener("click",(e)=>{
            if(e.target.className == "modal__card" || e.target.className == "visible"){
...
function HTMLModal(character,container){
    ...
    div.innerHTML = `   <img src="${character.image}" style="border-radius:8px 8px 8px 8px;">
                        <div class="modal__info">
                            <h2 class="modal__h2">${character.name}</h2>
                            <ul class="modal__ul">
                                <li class="modal__li">Status: ${character.status}</li>
                                <li class="modal__li">Species: ${character.species}</li>
                                <li class="modal__li">Type: ${character.type=="" ? "None" : character.type}</li>
                                <li class="modal__li">Gender: ${character.gender}</li>
                                <li class="modal__li">Origin: ${character.origin.name}</li>
                                <li class="modal__li pointer" id="location">Last location: ${character.location.name}</li>
                            </ul>
                        </div>`;
    addCon(container,div);
    document.querySelector("#location").addEventListener("click", ()=>{
        location.href="/Proyectos/cv/rickAndMorty/locations/index.html";
...
function HTMLPage(currentPage,pages,nextPage,prevPage,container){
    ...
    div.innerHTML=  `<ul class="pages__ul">
                    ${currentPage > 1 ? `<li class="pages__button" id="prev">Prev</li>` : ""}
                    <li class="pages__li">${currentPage}</li>
                    <p  class="pages__p">of</p>
                    <li class="">${pages}</li>
                    ${nextPage != null ? `<li class="pages__button" id="next"><a>Next</a></li>`:""}
                    </ul>`;
...
function listener(){
    ...
    rickAndMortyWiki.addEventListener("click", ()=>{
        window.location.href = "/Proyectos/cv/rickAndMorty/index.html";
    });
...
const api=async(status,gender,species,page,name,array,link)=>{
    ...
    if(array != undefined){url=`https://rickandmortyapi.com/api/character/${array}`;}
    else if(link != undefined){url=`${link}`}
    else{url=`https://rickandmortyapi.com/api/character/?page=${page}&status=${status}&gender=${gender}&species=${species}&name=${name}`;}
    ...
    if (array == undefined) {
        ...
        pages > 1 ? HTMLPage(currentPage,pages,nextPage,prevPage,pageContainer) : delCon(pageContainer);
    }
};
